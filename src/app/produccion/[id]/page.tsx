...
function ToastExito({ mensaje, onClose }: { mensaje: string; onClose: () => void }) {
    ...
    return (
        <div className="fixed bottom-6 right-6 z-50 flex items-center gap-3 bg-green-900/90 border border-green-500/40 text-green-300 px-5 py-3 rounded-xl shadow-2xl max-w-sm">
...
export default function OrdenProduccionDetallePage() {
    ...
    async function handleCambiarEstado(nuevoEstado: EstadoOP) {
        // Para COMPLETADA, usamos el flujo especial con loading
        if (nuevoEstado === 'COMPLETADA') {
            await handleCompletarOP();
            return;
        }
        ...
        if (result.error === 'STOCK_INSUFICIENTE' && result.faltantes) {
            ...
            if (confirm(alertMsg)) {
                await cambiarEstadoOrdenProduccion(ordenId, nuevoEstado, true);
                await loadData();
            }
    ...
    async function handleCompletarOP() {
        if (!orden) return;

        // Verificar que se ingresó cantidad producida
        const cantProd = parseFloat(cantidadProducida);
        if (!cantProd || cantProd <= 0) {
            alert('Ingresá la cantidad producida antes de completar la orden.');
            return;
        }

        // Armar resumen pre-confirmación
        const consumosConReal = orden.consumos?.filter(c => (c.cantidad_real || 0) > 0) ?? [];
        const consumosSinReal = orden.consumos?.filter(c => !(c.cantidad_real || 0)) ?? [];

        let resumen = `¿Completar Orden de Producción ${orden.numero}?\n\n`;
        resumen += `📦 Cantidad producida: ${cantProd} ${orden.unidad_medida}\n`;
        resumen += `🏭 Producto: ${orden.producto?.nombre || 'PT'}\n\n`;

        if (consumosSinReal.length > 0) {
            resumen += `⚠️ ${consumosSinReal.length} consumo(s) sin cantidad real registrada (se usará 0).\n\n`;
        }

        resumen += `Al confirmar:\n`;
        resumen += `• Se descontará el stock de ${consumosConReal.length} materia(s) prima\n`;
        resumen += `• Se sumará ${cantProd} ${orden.unidad_medida} al stock de ${orden.producto?.nombre || 'PT'}\n`;
        resumen += `• Se calculará el costo real y variación\n`;

        if (!confirm(resumen)) return;

        setCompletando(true);
        try {
            // Primero guardar la cantidad producida actualizada
            ...
            if (result.success) {
                await loadData();
                setToastMsg(
                    `✅ OP ${orden.numero} completada · ${cantProd} ${orden.unidad_medida} de ${orden.producto?.nombre || 'PT'} ingresados al stock`
                );
            } else {
                alert(`Error al completar la orden: ${result.error || 'desconocido'}`);
            }
        } catch (error) {
            console.error('Error completando OP:', error);
            alert('Error al completar la orden. Revisá la consola.');
        } finally {
    ...
    const consumosSinReal = enProduccion
        ? (orden.consumos?.filter(c => !(c.cantidad_real || 0)) ?? [])
        : [];
            ...
            <PageContainer
                ...
                actions={
                    <div className="flex gap-2">
                        ...
                        {enProduccion && (
                            <>
                                <Button variant="ghost" onClick={() => handleCambiarEstado('CANCELADA')} disabled={completando}>
                                ...
                                >
                                    {completando
                                        ...
                                        : <><Check className="w-4 h-4 mr-2" /> Completar OP</>
                                    }
                                </Button>
                            </>
                        )}
            ...
            >
                ...
                {enProduccion && (
                    <Card className="mb-6 border-[var(--color-warning)]/30">
                        ...
                        {/* Aviso consumos sin completar */}
                        {consumosSinReal.length > 0 && (
                            <div className="mt-3 p-3 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
                                <p className="text-xs text-yellow-400 font-medium">
                                    ⚠️ {consumosSinReal.length} consumo(s) sin cantidad real. Ingresalos en la tabla antes de completar la OP.
                                </p>
                            </div>
                        )}
                    </Card>
                ...
                <Card className="mb-6">
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-2">
                            <FlaskConical className="w-5 h-5 text-[var(--accent-gold)]" />
                    ...
                    {orden.consumos && orden.consumos.length > 0 ? (
                            ...
                            <table className="w-full text-sm">
                                ...
                                <tbody>
                                    {orden.consumos.map(consumo => {
                                        const sinReal = enProduccion && !(consumo.cantidad_real || 0);
                                        return (
                                            <tr
                                                key={consumo.id}
                                                className={`border-b border-[var(--border-default)]/50 ${sinReal ? 'bg-yellow-500/5' : ''}`}
                                            >
                                                <td className="py-2 px-3">
                                                    ...
                                                    {sinReal && (
                                                        <p className="text-xs text-yellow-500">Sin cantidad real</p>
                                                    )}
                                                </td>
                        ...
                        <div className="text-center py-8">
                            <Package className="w-10 h-10 mx-auto text-[var(--text-muted)] mb-2 opacity-30" />
                ...
                {completada && (
                    <Card className="mb-6 border-green-500/20 bg-green-900/5">
                        <div className="flex items-center gap-2 mb-4">
